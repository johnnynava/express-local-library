...
export const book_list = asyncHandler(async (req: Request, res: Response) => {
...
export const book_detail = asyncHandler(
  async (req: Request, res: Response, next: Next) => {
    ...
    if (book === null) {
      // No results.
      const err = new HTTPError("Book not found");
      err.status = 404;
      return next(err);
    }
...
export const book_create_post = [
  ...
  asyncHandler(async (req: Request, res: Response, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const [allAuthors, allGenres]: [any, any] = await Promise.all([
        Author.find().sort({ family_name: 1 }).exec(),
        Genre.find().sort({ name: 1 }).exec(),
      ]);
...
export const book_update_get = asyncHandler(
  async (req: Request, res: Response, next: Next) => {
    ...
    const [book, allAuthors, allGenres]: [any, any, any] = await Promise.all([
      Book.findById(req.params.id).populate("author").exec(),
      Author.find().sort({ family_name: 1 }).exec(),
      Genre.find().sort({ name: 1 }).exec(),
    ]);
